...
define([	'jquery', 'underscore', 'backbone', 'backbone.marionette',
			...
			'text!templates/ideas_list.html',
			'text!templates/ideas_list_row.html',
			
			'collections/ideas_list'
		], 
	function($,			_,			Bb,			Mn,
			tplRowEmpty,
			tplRowPreloader,
		...
		var output = Mn.View.extend({
			el: '#app',
			template: _.template(tplIdeasList),
			ui: {
			},
			events: {
			},
			...
			onRender: function(){
				if(this.collection.length === 0){
					...
					var RowItemView = Mn.View.extend({
						tagName: 'tr',
						template: _.template(tplIdeasListRow)
					});
