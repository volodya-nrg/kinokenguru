...
define([	'jquery', 'underscore', 'backbone', 'backbone.marionette', 
			'text!templates/quality_dubbing.html',
			'models/btns_cud',
			'views/btns_cud'
		], 
	function($,			_,			Bb,			Mn,						
		...
		var output = Mn.View.extend({
			el: '#app',
			template: _.template(tplQualityDubbing),
			ui: {
				...
				if(this.model.isValid() === false){
					...
					this.model.save(null, {
						success: function(model, response){
							...
							window.app.col.qualityDubbings.set(model, {remove: false});
							Bb.history.navigate('/admin/quality-dubbings', {trigger: true});
						},
			...
			delete: function(e){
				...
				this.model.destroy({
					success: function(model, response){
						...
						window.app.col.qualityDubbings.remove(model);
						Bb.history.navigate('/admin/quality-dubbings', {trigger: true});
					},
					error: function(model, response){
						$('#preloader').hide();
