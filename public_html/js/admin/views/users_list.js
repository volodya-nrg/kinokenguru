...
define([	'jquery', 'underscore', 'backbone', 'backbone.marionette',
			...
			'text!templates/users_list_row.html',
			
			'collections/users_list'
		], 
	function($,			_,			Bb,			Mn,
			tplRowEmpty,
			tplRowPreloader,
		...
		var output = Mn.View.extend({
			el: '#app',
			template: _.template(tplUsersList),
			ui: {
			},
			events: {
			},
			...
			onRender: function(){
				if(this.collection.length === 0){
					...
					var RowItemView = Mn.View.extend({
						tagName: 'tr',
						template: _.template(tplUsersListRow)
					});
